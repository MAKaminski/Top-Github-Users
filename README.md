...
site deploys the server. It is a **Claude connector**: public, read-only and unauthenticated, so
adding it is a URL and nothing else.
...
| Claude Code | `claude mcp add --transport http commitgraph https://top-github-users-amber.vercel.app/api/mcp` |
| Claude Code, packaged | `/plugin marketplace add MAKaminski/Top-Github-Users` then `/plugin install commitgraph@commitgraph` |
| Anything else | `{"mcpServers":{"commitgraph":{"type":"http","url":"…/api/mcp"}}}` |

[`/connect`](https://top-github-users-amber.vercel.app/connect) is the page to send people to; it
is generated from the server's own tool and prompt registries, so it cannot advertise something
...
Five prompts (`lib/mcp/prompts.ts`) package the multi-tool sequences that would otherwise have to
be assembled by hand: `commitgraph_orient`, `commitgraph_place_report`, `commitgraph_scout`,
`commitgraph_compare` and `commitgraph_audit_ranking`. Each returns a single **user** message —
never a fabricated assistant turn — and each restates the measured-versus-estimated rule, because
a prompt may be the first thing a session sees.

### Claude Code plugin — `plugins/commitgraph/`

The connector packaged as an installable add-in, catalogued by
[`.claude-plugin/marketplace.json`](.claude-plugin/marketplace.json) at the repository root. It
bundles the hosted MCP server plus a skill that tells Claude which tool answers what and which
four properties of this data make an answer wrong if ignored. `COMMITGRAPH_MCP_URL` retargets it
at a fork or a local `pnpm dev`.

The server itself is remote rather than bundled on purpose: a local copy of the snapshot goes
stale the moment it is installed, and the data is regenerated by a scheduled crawl.
`scripts/packaging.test.ts` holds the manifests, the marketplace entry, the plugin's MCP config
and `lib/site.ts` to the same URL and version, because every way this drifts fails silently.
